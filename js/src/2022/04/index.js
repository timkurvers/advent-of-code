...
const isContaining = (pair) => {
  const [one, two] = pair;
  return (one.start <= two.start && one.end >= two.end) || (two.start <= one.start && two.end >= one.end);
};

const isOverlapping = (pair) => {
  const [one, two] = pair;
  return (one.start >= two.start && one.start <= two.end) || (two.start >= one.start && two.start <= one.end);
};
